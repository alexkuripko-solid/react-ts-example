...
const RebookAppointment = (): ReactElement | null => {
  ...
  const handleBookingConfirm = () => {
    navigate(
      `${ClientRouteEnum.BOOKINGS}?tab=${
        BookingsTabs.WAITING_FOR_PAYMENT
      }&openPaymentModal=1&selected=[${addedAppointments.map(({ id }) => id).join(',')}]`,
    );
  };
  ...
  return (
    <Grid sx={{ p: isMobile ? 2 : 4 }} container spacing={2} maxWidth={theme.breakpoints.values.xl}>
        ...
        <Card>
          <Grid container>
            <Grid item xs={12} md={8}>
              <Box display="flex">
                <Avatar src={getImagePath(provider.details.image_id || 0)} />
                <Box>
      ...
      <Grid item xs={12}>
        <Card>
          <Grid container>
            ...
            <Grid
              sx={{ borderLeft: `1px solid ${colors.secondary[30]}`, pl: 2 }}
              position="relative"
              item
              xs={12}
              md={4}
      ...
      <Grid id="reviewsSection" item xs={12}>
        <Typography sx={{ mt: 2, mb: 2 }} fontWeight={600}>Reviews</Typography>
        <ReviewsList
