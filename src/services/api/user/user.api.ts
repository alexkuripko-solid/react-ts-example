...
class UserApiService extends ApiService {
  ...
  update = async (id: number, user: IUser): Promise<IUser> => await this.put(`/${id}`, user, false).then((res) => res.data);

  deleteById = async (id: number): Promise<IUser> => await this.delete(`/${id}`).then((res) => res.data);
  ...
  applyEmailVerificationCode = async (userId: number, code: string): Promise<AxiosResponse> =>
    await this.post(`/apply-verification-code/${userId}/${code}`).then((res) => res.data);
