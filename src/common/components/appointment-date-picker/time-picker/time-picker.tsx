...
const TimePicker = ({ row = false, type, selectedIntervals, onIntervalsChange, onTypeChange, header }: Props): ReactElement => {
  ...
  const handleDelete = (index: number) => {
    onIntervalsChange([
      ...selectedIntervals.slice(0, index),
      ...selectedIntervals.slice(index + 1),
    ]);
  };
  ...
  return (
    <Box>
      ...
      <RadioGroup
        row={row}
        value={type}
        onChange={(e, value) => onTypeChange(parseInt(value))}
      >
        {
          Object.entries(AppointmentTimeTypeLabels).map(([value, label]) => (
            <FormControlLabel
              key={value}
