...
const Item = ({ service, selected, onSelect }: ItemProps) => {
  ...
  const imageUrl = getImagePath(service.image_id || 0);
  ...
  return (
    ...
    >
      <GridContainer container spacing={1}>
        <ItemCardInfo item lg={5} md={7} sm={12}>
          <Image sx={{ minWidth: 100, height: 100 }} src={imageUrl} />
          <Box>
            <Title>{service.name}</Title>
            <Description>{expanded ? service.description : `${service.description.slice(0, 150)}...`}</Description>
          </Box>
        </ItemCardInfo>
        <Actions item lg={7} md={5} sm={12} xs={12}>
          <StyledButton
            variant={'contained'}
