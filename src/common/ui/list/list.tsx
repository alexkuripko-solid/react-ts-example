...
const Placeholder = ({ length, actions }: PlaceholderProps): ReactElement => {
  const count = Array.from({ length }, (_, index) => index + 1);

  return (
    <>
      {count.map((index) => (
        <ListItemSkeleton actions={actions} key={index} />
      ))}
    </>
  );
};
...
const List = ({
  ...
  if (!loading && !data.length) {
    return <EmptyPage title={emptyPageTitle} buttonLabel={emptyPageButtonLabel} onClick={emptyPageOnClick} />;
  }

  return (
    <Box>
      {loading ? (
        <Placeholder actions={actions} length={rowsPerPage} />
      ) : (
        data.map((item) => {
          return (
      ...
      {selectedItems?.length && onSelect ? (
        <Grid container sx={{ mt: 3, mb: 3 }}>
          <Grid item xs={12} md={6}>
            <Button
              sx={{ width: isMobile ? '100%' : 300, mb: 2 }}
              variant="outlined"
              onClick={() => onSelect([])}
            >
              Сancel selection
